...
class Solution:
    ...
    def spath_algo(self, graph):
        ...
        visited = []
        return self.spath_recursive(graph, 'Start', visited)
    def spath_recursive(self, graph, node, visited):
        if node in visited:
            return 9999
        else:
            visited.append(node)
        if node == 'Finish':
            ...
            for n in graph[node].keys():
                visited_copy = [x for x in visited]
                val = self.spath_recursive(graph, n, visited_copy) + graph[node][n]
                if val < min:
                    min = val
            return min
